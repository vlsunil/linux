					   ...
					   u32 *gsi_base, u32 *id)
{
	...
	if (!is_of_node(dev->fwnode)) {
		acpi_status status;
		u32 plic_id, num_sources;
		u64 gbase;

		if (!acpi_has_method(ACPI_HANDLE(dev), "_GSB"))
			return -1;

		status = acpi_evaluate_integer(ACPI_HANDLE(dev), "_GSB", NULL, &gbase);
		if (ACPI_FAILURE(status)) {
			acpi_handle_err(ACPI_HANDLE(dev), "failed to evaluate _GSB method\n");
				return -1;
		}

		plic_id = parse_madt_plic_entry((u32)gbase, &num_sources);
		if (plic_id < 0) {
			dev_err(dev, "failed to find PLIC in MADT\n");
			return -1;
		}
		*gsi_base = (u32)gbase;
		*nr_irqs = num_sources;
