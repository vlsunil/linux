...
EXPORT_SYMBOL_GPL(gen_raw_pci_read);
...
EXPORT_SYMBOL_GPL(gen_raw_pci_write);
...
EXPORT_SYMBOL_GPL(gen_acpi_pci_bus_find_domain_nr);
...
static int pci_acpi_root_prepare_resources(struct acpi_pci_root_info *pri)
{
	struct resource_entry *entry, *tmp;
	int status;

	status = acpi_pci_probe_root_resources(pri);
	resource_list_for_each_entry_safe(entry, tmp, &pri->resources) {
 ...
 * mapping.
 */
static struct pci_config_window *pci_acpi_setup_ecam_mapping(struct acpi_pci_root *root)
{
...
/*
 * Free resources allocated by init_info
 */
static void pci_acpi_generic_release_info(struct acpi_pci_root_info *pri)
{
	struct acpi_pci_generic_root_info *ri;

	ri = container_of(pri, struct acpi_pci_generic_root_info, common);
...
EXPORT_SYMBOL_GPL(gen_pci_acpi_scan_root);
#endif
