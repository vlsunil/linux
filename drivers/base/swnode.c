...
software_node_to_swnode(const struct software_node *node)
{
	...
	list_for_each_entry(swnode, &swnode_early_list, early_head) {
		if (swnode->node == node) {
		...
		unsigned int allocated, unsigned int early)
{
	...
	INIT_LIST_HEAD(&swnode->early_head);
	...
	if (early) {
		...
		list_add_tail(&swnode->early_head, &swnode_early_list);
	} else {
		if (node->name)
